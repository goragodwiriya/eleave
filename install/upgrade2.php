        ...
        if (!$error) {
            ...
            try {
                ...
                if (!$db->fieldExists($table, 'reason')) {
                    $db->query("ALTER TABLE `$table` ADD `reason` VARCHAR(255) COLLATE utf8_unicode_ci DEFAULT NULL");
                }
                $db->query("ALTER TABLE `$table` CHANGE `department` `department` VARCHAR(10) DEFAULT NULL");
                ...
                if (is_string($config['eleave_fiscal_year']) && preg_match('/^Y\-([0-9]+)\-01$/', $config['eleave_fiscal_year'], $match)) {
                    $config['eleave_fiscal_year'] = (int) $match[1];
                }
