    ...
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        if (!$error) {
            ...
            try {
                ...
                // leave
                // =========================================================
                $table_leave = $db_config['prefix'].'_leave';
                // migrate published → is_active
                if (!$db->fieldExists($table_leave, 'is_active')) {
                    $db->query("ALTER TABLE `$table_leave` ADD `is_active` TINYINT(1) NULL");
                    if ($db->fieldExists($table_leave, 'published')) {
                        $db->query("UPDATE `$table_leave` SET `is_active` = `published`");
                    } else {
                        $db->query("UPDATE `$table_leave` SET `is_active` = 1");
                    }
                    $db->query("ALTER TABLE `$table_leave` MODIFY `is_active` TINYINT(1) NOT NULL DEFAULT 1");
                    $content[] = '<li class="correct">leave: เพิ่ม is_active</li>';
                }
                if ($db->fieldExists($table_leave, 'published')) {
                    $db->query("ALTER TABLE `$table_leave` DROP COLUMN `published`");
                    $content[] = '<li class="correct">leave: ลบ published</li>';
                }

                // =========================================================
                // leave_items
                // =========================================================
                $table_leave_items = $db_config['prefix'].'_leave_items';

                if ($db->fieldExists($table_leave_items, 'create_date')) {
                    $db->query("ALTER TABLE `$table_leave_items` CHANGE `create_date` `created_at` DATETIME NULL DEFAULT NULL");
                    $content[] = '<li class="correct">leave_items: เปลี่ยนชื่อ create_date → created_at</li>';
                }
